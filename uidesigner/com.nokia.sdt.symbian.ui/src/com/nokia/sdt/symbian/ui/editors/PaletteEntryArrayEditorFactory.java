...
public class PaletteEntryArrayEditorFactory extends AbstractArrayPropertyEditorFactory {
	...
	private static void initSysColorMaps() {
		if (sysColorToLocalized == null) {
			...
			noneDisplayString = Messages.getString("PaletteEntryEditorFactory.1"); //$NON-NLS-1$
		}
	}
	...
	class PaletteEntryLabelProvider extends LabelProvider {

    	private ILookAndFeel laf;
    	private Image lastImage;
    	private Image sysColorImage;
    	
    	PaletteEntryLabelProvider(EObject object) throws CoreException {
    		initSysColorMaps();
	    ...
	    public Image getImage(Object element) {
	    	Image ColorImage;
	    	ColorImage = makeRGBImage(new RGB(255, 255, 255), true);
	    	return ColorImage;
	    	}

	    public String getText(Object element) {
	    	IPropertySource palettePropertySource = (IPropertySource) element;
	    	return palettePropertySource.getPropertyValue("label").toString(); //$NON-NLS-1$
	    	}
	}

	   static class PaletteEntryCellEditorValidator implements ICellEditorValidator {
	    	
	    	PaletteEntryCellEditorValidator() {
	    		initSysColorMaps();
	    	}
			...
			public String isValid(Object value) {
				...
				String result = null;
				if (!valid) {
					result = Messages.getString("PaletteEntryEditorFactory.3"); //$NON-NLS-1$
				}
				return result;
			}
	...
	public ICellEditorValidator createCellEditorValidator(EObject object, String propertyId) {
		return new PaletteEntryCellEditorValidator();
	}
	...
	public ILabelProvider createElementLabelProvider(final EObject object, Object propertyID) {
		...
		try {
			result = new PaletteEntryLabelProvider(object);

...
QueryDialog.prototype.draw = function(instance, laf, graphics) {
...
QueryDialog.prototype.getPreferredSize = function(instance, laf, wHint, hHint) {
	return null;
}
...
function draw(instance, laf, graphics) {
	...
	var pixelGapBetweenLines = laf.getInteger("queryDialog.lineGap", 0); 
...
function renderText ( instance, laf, graphics, text, rectX, rectY, rectWidth, rectHeight, pixelGapBetweenLines ) {
	...
	var buttonRect =  laf.getRectangle("queryDialog.button");
	var height = rectHeight - buttonRect.height;
	var noTextTopHeight = getNoTextHeight( laf ) - getNoTextBottomHeight( laf );

	var image = laf.getImage("queryDialog.content.background");
	var imageScroll = laf.getImage("queryDialog.scrollContent.background");
	...
	if ( (rectHeight == (container.height - container.y)) && ( imageScroll != null) ) {
		var titleRect =  laf.getRectangle("queryDialog.title");
	...
	rectX = laf.getInteger("queryDialog.left.text.margin", 12); 
...
function renderButtonText ( instance, laf, graphics, rectHeight, pixelGapBetweenLines ) {
	...
	var titlePadding = laf.getInteger("queryDialog.title.padding", 5); 
	var buttonRect =  laf.getRectangle("queryDialog.button");
	buttonRect.y = rectHeight - buttonRect.height;
	var image = laf.getImage("queryDialog.button.background");
	if (image != null) {
...
function renderTitle( instance, laf, graphics, font, title, pixelGapBetweenLines) {
	...
	var image = laf.getImage("queryDialog.title.background");
	if (image != null) {
...
function renderIcon(instance, laf, graphics) {
	var container = searchTopLevelContainerRectangle(instance, laf);
	var titleRectRightMargin = laf.getRectangle("queryDialog.title.rectangle.right.margin");
	var titleRect =  laf.getRectangle("queryDialog.title");

	var iconRect = laf.getRectangle("queryDialog.icon");
	var iconWidth = iconRect.width;
	var iconHeight = iconRect.height;
	var iconX = ( container.width - titleRectRightMargin - iconWidth ) / 2;
	var iconY = laf.getInteger("queryDialog.top.margin", 6) + 
				titleRect.height + 
				laf.getInteger("queryDialog.titleIcon.margin", 8);
	var image = laf.getImage("queryDialog.icon");
	if (image != null) {
...
function searchTopLevelContainerRectangle(instance, laf) {
	...
	var VIEW_KEY = "view.queryDialog.bounds";
	var SIMPLE_DIALOG_ID = "com.nokia.carbide.uiq.CQikSimpleDialog";
	var SIMPLE_DIALOG_KEY = "simpleDialog.queryDialog.bounds";
...
setupCommonDirectLabelEditing(QueryDialog.prototype,
	"text",
	areaWithParentWidth, // areafunction
	QueryDialog.prototype.getFont
);

QueryDialog.prototype.getFont = function(instance, laf) {
...
QueryDialog.prototype.layout = function(instance, laf) {
...
function getNoTextHeight( laf ) {
	var titleRect =  laf.getRectangle("queryDialog.title");
	var iconRect = laf.getRectangle("queryDialog.icon");
	var topMargin = laf.getInteger("queryDialog.top.margin", 6); 
	var titleIconMargin = laf.getInteger("queryDialog.titleIcon.margin", 8); 
	var iconTextMargin = laf.getInteger("queryDialog.iconText.margin", 8); 
...
function getNoTextBottomHeight( laf ) {
	var buttonRect = laf.getRectangle("queryDialog.button");
	var textButtonMargin = laf.getInteger("queryDialog.textButton.margin", 7); 
...
function getLeftRightTextMargin ( laf ) {
	var leftTextMargin = laf.getInteger("queryDialog.left.text.margin", 12); 
	var rightTextMargin = laf.getInteger("queryDialog.right.text.margin", 21); 
...
QueryDialog.prototype.propertyChanged = function(instance, property) {

	...
	prototype.drawContent = function(instance, laf, graphics, rect) {
		...
		if (color == null) {
			if (!isControl(instance.parent))
				color = laf.getColor("listitem.text");
			else
		...
		if (instance.isInstanceOf("com.nokia.sdt.series60.CEikEdwinBase"))
			text = text.replace(/\n/,"");
		else
...
function isControl(instance) {
	return instance.componentId == "com.nokia.sdt.series60.CCoeControl";
}

function getEmbeddedEditorFont(instance, laf) {
	// This flag denotes 3.1+
	if (!laf.getBoolean("decorations", true)) {
		if (isSettingItemList(instance.parent))
			return laf.getFont("EditorFont.settings.list");
		else
...
function getStandardEditorFont(instance, laf) {
	if (!isControl(instance.parent))
		return getEmbeddedEditorFont(instance, laf);
	else
		return laf.getFont("EditorFont");
}

function getEditorFontWithFontProperty(instance, laf) {
	if (!isControl(instance.parent))
		return getEmbeddedEditorFont(instance, laf);

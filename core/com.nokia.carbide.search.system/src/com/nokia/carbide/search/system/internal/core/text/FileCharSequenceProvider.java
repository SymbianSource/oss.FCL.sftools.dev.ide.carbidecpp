...
public class FileCharSequenceProvider {
	...
	public CharSequence newCharSequence(IFileStore file) throws CoreException, IOException {
		if (fReused == null) {
	...
	private final class FileCharSequence implements CharSequence {
		...
		public FileCharSequence(IFileStore file) throws CoreException, IOException {
			fNumberOfBuffers= 0;
			reset(file);
		}
		
		public void reset(IFileStore file) throws CoreException, IOException {
			fFile= file;
...
//			fReader= new InputStreamReader(getInputStream(charset), charset);
			fReader= new InputStreamReader(fFile.openInputStream(EFS.NONE, null));
			fReaderPos= 0;
		}
		
		private InputStream getInputStream(String charset) throws CoreException, IOException {
			boolean ok= false;
			InputStream contents= fFile.openInputStream(EFS.NONE, null);
//			try {

...
public class FileSearchResult extends AbstractTextSearchResult implements IEditorMatchAdapter, IFileMatchAdapter {
	...
	public Match[] computeContainedMatches(AbstractTextSearchResult result, IEditorPart editor) {
		...
		if (ei instanceof IFileEditorInput) {
			...
			URI uri = fi.getURI();
			String comparePath = uri.getPath();
			
			// check for a Windows drive letter path format
			if (   (comparePath != null)
				&& (comparePath.charAt(0) == '\\' || comparePath.charAt(0) == '/')
				&& (comparePath.charAt(2) == ':'))
				comparePath = comparePath.substring(1);
			
			return getMatches(comparePath);
		}

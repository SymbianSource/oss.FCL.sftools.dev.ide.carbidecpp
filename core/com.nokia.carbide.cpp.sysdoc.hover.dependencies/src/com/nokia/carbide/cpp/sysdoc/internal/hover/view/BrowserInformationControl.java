...
public class BrowserInformationControl extends AbstractInformationControl
	...
	private volatile boolean fBrowserHasContent;
	...
	private IBrowserInformationControlAgent browserInformationControlAgent = null;
	 ...
	 * @param able
	 *            <code>true</code> if the control should be able
	 * @since 3.4
	 */
	public BrowserInformationControl(Shell parent, String symbolicFontName,
			boolean able,
			IBrowserInformationControlAgent browserInformationControlAgent) {
		super(parent, able);
		this.browserInformationControlAgent = browserInformationControlAgent;
	...
	protected void createContent(Composite parent) {
		browserInformationControlAgent.setShell(getShell());
		browserInformationControlAgent.createContent(parent);
		fBrowser = browserInformationControlAgent.getBrowser();
	...
	public void setVisible(boolean visible) {
		...
		if (shell.isVisible() == visible)
			return;
		browserInformationControlAgent.setVisible();
		if (!visible) {
	...
	public void setSize(int width, int height) {
		// When mouse over the popup menu, it will get bigger ...
		if (w == -1) {
			w = width + 100;
			h = height + 50;
		}
	...
	public Point computeSizeHint() {
		...
		// NOTE: The HTML2TextReader does not render <p> like a browser.
		...
		HTML2TextReader reader = new HTML2TextReader(new StringReader(fInput
				.getHtml()), presentation);
		String text;
		try {
		...
		Rectangle bounds = fTextLayout.getBounds(); // does not return minimum
		// width, see
	...
	public Rectangle computeTrim() {
		...
		if (isResizable()) {
			...
			trim.width += fgScrollBarSize.x + 350;
			trim.height += fgScrollBarSize.y + 20;
		}
		return trim;
	}
	...
	/**
	 * setInput method is not powerful enough if we want to refer to a online
	 * address
	 * 
	 * @param url
	 *            : address to refer browser
	 */
